...
namespace ini_config
{
	...
	struct string_container
	{
		...
		{
			auto dst = data;
			for (auto src = s; src != s + N; ++src) { *dst++ = *src; }
		}


		consteval operator const char_type*() const noexcept { return data; }

		consteval auto size() const noexcept { return N; }

		consteval auto begin() const noexcept { return data; }

		consteval auto end() const noexcept { return data + N; }
	};
	...
	class ini_config
	{
		...
		consteval static bool isgraph(char_type c) noexcept { return c > ' ' && c != 0x7F; }

		consteval static bool iseol(char_type c) noexcept { return c == '\n' || c == '\0'; }

		consteval static bool iscomment(char_type c) noexcept { return c == ';' || c == '#'; }
		...
		constexpr static int_type from_string(const char_type* str) noexcept
		{
			...
			bool neg = *str == '-';
			if (neg) { ++str; }
			for (; *str && *str >= '0' && *str <= '9'; ++str) { ret = ret * 10 + (*str - '0'); }
			return !neg ? ret : -ret;
		}
		...
		consteval static unsigned int verify_and_size()
		{
			...
			do
			{
				...
				if (iseol(*p) || iscomment(*p)) { continue; }
				...
				if (*p == '[')
				{
					...
					while (!iseol(*p) && *p != ']');
					if (iseol(*p)) { throw "Bad section tag!"; }
				...
				for (bool keyend = false; !iseol(*p); ++p)
				{
					...
					if (!keyend)
					{
						if (!isgraph(*p)) { keyend = true; }
						else { ++sizechars; }
					}
					else { if (isgraph(*p)) { throw "Invalid key!"; } }
				}
				if (*p != '=') { throw "Invalid key!"; }
				++p;
				...
				{
					if (!valuestart && isgraph(*p)) { valuestart = true; }
					if (valuestart) { ++sizechars; }
				}
				if (oldsizechars == sizechars) { throw "No value!"; }
		...
		consteval static unsigned int kvpcount() noexcept
		{
			...
			do
			{
				...
				if (iseol(*p) || iscomment(*p) || *p == '[') { continue; }
	...
	public:
		...
		public:
			...
			{
				if (auto poscomp = m_pos <=> other.m_pos; poscomp != 0) { return poscomp; }
				if (m_current.first == nullptr) { return std::strong_ordering::greater; }
				if (other.m_current.first == nullptr) { return std::strong_ordering::less; }
		 ...
		 */
		consteval auto size() const noexcept { return kvpcount(); }

		constexpr auto begin() const noexcept { return iterator(kvp_buffer); }
		...
		{
			auto it = begin();
			do { if (it->section != nullptr && stringmatch(it->section, section)) { break; } }
			while (++it != end());
			return it;
		}
		 ...
		 */
		constexpr auto section(const char_type* s) const noexcept { return section_view(*this, s); }
		...
		{
			for (auto kvp : *this) { if (stringmatch(kvp.first, key)) { return kvp.second; } }
			return "";
		}
		...
		consteval T get(const char_type* key) const noexcept { return from_string<T>(get(key)); }
		...
		{
			for (auto kvp : section(sec)) { if (stringmatch(kvp.first, key)) { return kvp.second; } }
			return "";
		}
		...
		template <typename T>
			requires(std::integral<T> || std::floating_point<T>)
		T tryget(const char_type* key) const noexcept { return from_string<T>(tryget(key)); }
		...
		template <typename T>
			requires(std::integral<T> || std::floating_point<T>)
		T tryget(const char_type* sec, const char_type* key) const noexcept { return from_string<T>(tryget(sec, key)); }


		consteval bool contains(const char_type* key) const noexcept { return *get(key) != '\0'; }
		...
		bool trycontains(const char_type* key) const noexcept { return *tryget(key) != '\0'; }
...
consteval auto operator ""_ini() { return ini_config::ini_config<Input>(); }
