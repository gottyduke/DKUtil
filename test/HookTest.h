...
namespace Test::Hook
{
	...
	void RecalculateCombatRadius(bool a_fullRadius, float* a_radius, float a_delta, RE::Character* a_center, RE::Character* a_actor)
	{
	...
	// 1-6-353-0 @ 0x870CF0
	constexpr std::uint64_t AE_FuncID = 50643;
	// 1-5-97-0 @ 0x844E40
	constexpr std::uint64_t SE_FuncID = 49716;

	constexpr std::ptrdiff_t RadiusL = 0x99;
	constexpr std::ptrdiff_t RadiusH = 0xA0;
	constexpr std::ptrdiff_t MedianL = 0x139;
	constexpr std::ptrdiff_t MedianH = 0x145;

	constexpr Patch RadiusPatch{
		// lahf
		"\x9F\x50"
		// mov rdx, rdi
		"\x48\x89\xFA"
	...
	constexpr Patch MedianPatch{
		// xor cl
		"\x30\xC9",
		19
	};

	constexpr Patch Epilog{
		// sahf
		"\x58\x9E",
		2
	};
	...
	void Install() 
	{
		...
		DKUtil::model::enumeration<CaveFlag> flag = CaveFlag::kRestoreAfterEpilog;
		flag.set(CaveFlag::kRestoreBeforeProlog, CaveFlag::kRestoreAfterProlog);
		...
		/**/
		RescaleCircleChanceHook::InstallHook();
	}
}
