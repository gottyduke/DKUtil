		...
		{
			{
				...
				dku_assert(std::ranges::equal(std::span{ prolog.first, prolog.second }, expected_prolog),
					"prolog incorrect");
				dku_assert(std::ranges::equal(std::span{ epilog.first, epilog.second }, expected_epilog),
					"epilog incorrect");
			}
